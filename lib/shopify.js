...
export async function publishToChannels(productId, publicationIds) {
  if (!productId || !publicationIds || publicationIds.length === 0) return true;
  ...
  const input = publicationIds.filter(Boolean).map(publicationId => ({ publicationId }));
  if (input.length === 0) return true;

  try {
    const result = await shopifyGraphQL(mutation, { id: productId, input });
    if (result.data?.publishablePublish?.userErrors?.length > 0) {
      console.warn('[publishToChannels] warnings:', result.data.publishablePublish.userErrors);
    }
    return true;
  } catch (error) {
    console.error('[publishToChannels] failed:', error.message);
    throw error;
  }
}
